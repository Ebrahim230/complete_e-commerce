...
const PlaceOrder = () => {
  ...
  const { navigate, serverUrl, token, cartItems, setCartItems, getCartAmount, delivery_fee, products } = useContext(ShopContext);
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      for (const items in cartItems) {
        for (const item in cartItems[items]) {
          if (cartItems[items][item] > 0) {
            ...
            if (itemInfo) {
              itemInfo.size = item;
              itemInfo.qunatity = cartItems[items][item];
      ...
      switch (method) {
        case 'cod':
          const res = await axios.post(serverUrl + '/api/order/place', orderData, { headers: { token } });
          if (res.data.success) {
            setCartItems({});
            navigate('/orders');
          } else {
          ...
          break;
        case 'stripe':
          const resStripe = await axios.post(serverUrl + '/api/order/stripe', orderData, { headers: { token } });
