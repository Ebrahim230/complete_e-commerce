...
const Collection = () => {
  ...
  const applyFilter = () => {
    ...
    if (subCategory.length > 0) {
      productsCopy = productsCopy.filter(i => subCategory.includes(i.subCategory));
    }
    ...
    <div className='flex flex-col sm:flex-row gap-1 sm:gap-10 pt-10 border-t '>
      ...
      <div className='flex-1'>
        ...
        <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 gap-y-6'>
          {
            filterProducts.map((i, j) => (
              <ProductItem key={j} name={i.name} id={i._id} price={i.price} image={i.image} />
            ))
          }
        </div>
      </div>
